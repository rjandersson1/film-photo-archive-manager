...
class Renderer:
    def __init__(self, roll):
        ...
        self.rebate_image_path = None
        self.rebate_image = None 
        self.rebate_width = None
        self.rebate_height = None
        self.prepare_frame()
    ...
    def build_grid(self):
        ...
        self.grid = []
        self.grid_centered = []
        self.rows = math.ceil(self.frame_count / self.cols)
    ...
    def render_images(self):
        roll = self.roll
        if roll.filmformat == '35mm':
            frame_width = self.px(36)
            frame_height = self.px(24)
            vertical_offset = self.px(2.5)
        if roll.filmformat == '6x7':
            frame_width = self.px(70 * SCALE_120)
            frame_height = self.px(56 * SCALE_120)
            vertical_offset = self.px(2.5)
    ...
    def prepare_frame(self):
        roll = self.roll
        p = None
        if roll.isColor and roll.filmformat == '35mm': p = 'data/film_rebates/135-color.png'
        if roll.isColor and roll.filmformat == '6x7': p = 'data/film_rebates/120-color-6x7.png' 
        if roll.isBlackAndWhite and roll.filmformat == '35mm': p = 'data/film_rebates/135-bw.png'
        if roll.isBlackAndWhite and roll.filmformat == '6x7': p = 'data/film_rebates/120-bw-6x7.png' 
        if p is None:
            print("error in prepareFrame() in renderTool")
        else:
            self.rebate_image_path = p
        img = Image.open(p)
        
        w, h = img.size
        scale = SCALE_120
        self.rebate_width = w
        self.rebate_height = h + self.px(2.5) # can add buffer here
        ...
        if roll.filmformat == '6x7':
            self.cols = 3
            self.rebate_header_coords = (874*SCALE_120, 37*SCALE_120, 0, 0)
            self.rebate_footer_coords = (425*SCALE_120, 27*SCALE_120, 0, 698*SCALE_120) # w,h,x,y
